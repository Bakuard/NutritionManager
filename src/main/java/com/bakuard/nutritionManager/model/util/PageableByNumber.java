...
public final class PageableByNumber implements Pageable {
     ...
     * которая должна содержать элемент с указанным порядковым номером (нумерация начинается с нуля).
     * Надо напомнить что выборка элементов осуществляется из упорядоченного множества элементов,
     * правила сортировки для которого задаются отдельно.
     * @param expectedPageSize размер страницы. Фактический размер страницы может отличаться в следующих случаях:<br/>
    ...
    /**
     * Возвращает ожидаемый размер страницы, которая будет получена с помощью данного объекта Pageable.
     * @return ожидаемый размер страницы, которая будет получена с помощью данного объекта Pageable.
     */
    @Override
    ...
    /**
     * Возвращает ожидаемый номер страницы, которая будет получена с помощью данного объекта Pageable.
     * @return ожидаемый номер страницы, которая будет получена с помощью данного объекта Pageable.
     */
    ...
    /**
     * Создает и возвращает объект представляющий метаданные исходной выборки будущей страницы. Создание метаданых
