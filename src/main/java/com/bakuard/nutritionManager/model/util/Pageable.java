...
public final class Pageable {
     ...
     * @param expectedPageNumber порядковый номер продукта по которому определяется ожидаемый номер страницы.
     ...
     */
    public static Pageable ofIndex(int expectedPageSize, int expectedPageNumber) {
        return ofIndex(expectedPageSize, BigInteger.valueOf(expectedPageNumber));
    }
     ...
     * @return новый объект Pageable.
     */
    public static Pageable ofIndex(int expectedPageSize, BigInteger expectedPageNumber) {
        expectedPageSize = Math.max(1, expectedPageSize);
        expectedPageNumber = expectedPageNumber.max(BigInteger.ZERO).
                divide(BigInteger.valueOf(expectedPageSize));

        return of(expectedPageSize, expectedPageNumber);
    }
