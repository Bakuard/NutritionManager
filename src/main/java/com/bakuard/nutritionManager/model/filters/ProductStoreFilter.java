...
/**
 * Фильтр хранилища продуктов предназначен для задания следующих ограничений: какие продукты из имеющихся у
 * пользователя в наличии и в каком кол-ве могут использоваться. Объекты данного класса являются неизменяемыми.
 */
public final class ProductStoreFilter {

    /**
     * Возвращает фильтр указывающий, что можно использовать любые продукты из имеющихся у пользователя
     ...
     * использовать.
     * @return фильтр задающий пустое множество для продуктов имеющихся у пользователя в наличии.
     */
    ...
    /**
     * Создает фильтр задающий универсальное множество для всех продуктов имеющихся у пользователя в
     * наличии.
     */
     ...
     * из имеющихся в наличии у пользователя продуктов разрешено использовать продукт с данным идентификатором и
     ...
     * @param quantity кол-во в котором разрешено использовать указанный продукт.
     * @return новый объект, если указанный продукт отсутствует в текущем фильтре или указанное кол-во
     *         отличается от такового в текущем фильтре, иначе - этот же объект.
     * @throws ValidateException если productId или quantity имеют значение null.
     */
    ...
    /**
     * Возвращает кол-во в котором разрешено использовать продукт с указанным идентификатором.
     * @param productId идентификатор искомого продукта.
     * @return кол-во в котором разрешено использовать продукт с указанным идентификатором.
     */
